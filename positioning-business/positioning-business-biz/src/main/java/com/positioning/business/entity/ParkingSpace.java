 ...
 * 车位表: 商场内停车位，含几何与状态
 */
@Schema(description = "车位表：商场内停车位，含几何与状态")
@Data
@TableName("parking_space")
public class ParkingSpace {

    /** 车位ID（雪花ID，应用层生成） */
    @Schema(description = "车位ID（雪花ID，应用层生成）")
    ...
    @Schema(description = "车位类型: NORMAL=普通 DISABLED=无障碍 CHARGING=充电 COMPACT=微型 MECHANICAL=机械 MOTHER_CHILD=母婴")
    private String spaceType;

    /** 车位状态: FREE=空闲 OCCUPIED=占用 LOCKED=锁定 FAULT=故障 */
    @Schema(description = "车位状态: FREE=空闲 OCCUPIED=占用 LOCKED=锁定 FAULT=故障")
    private String status;

    /** 占用信息来源: APP/CAMERA/MAGNET/GATE/MANUAL */
    @Schema(description = "占用信息来源: APP/CAMERA/MAGNET/GATE/MANUAL")
    private String occupySource;

    /** 车位轮廓（PostGIS几何, GeoJSON; 通用CRUD不读写, 走专门空间接口） */
    @Schema(description = "车位轮廓（PostGIS几何, GeoJSON; 通用CRUD不读写, 走专门空间接口）")
    ...
    @Schema(description = "车位中心点（PostGIS几何, GeoJSON; 通用CRUD不读写, 走专门空间接口）")
    @TableField(select = false, insertStrategy = FieldStrategy.NEVER, updateStrategy = FieldStrategy.NEVER)
    private String centerPoint;

    /** 车位开口点（PostGIS几何, GeoJSON; 通用CRUD不读写, 走专门空间接口） */
    @Schema(description = "车位开口点（PostGIS几何, GeoJSON; 通用CRUD不读写, 走专门空间接口）")
    @TableField(select = false, insertStrategy = FieldStrategy.NEVER, updateStrategy = FieldStrategy.NEVER)

 ...
 */
@Schema(description = "基础设施/兴趣点表: 电梯、扶梯、楼梯、卫生间、出入口、服务台等")
@Data
@TableName("poi")
public class Poi {

    /** 设施ID（雪花ID，应用层生成） */
    @Schema(description = "设施ID（雪花ID，应用层生成）")
    ...
    @Schema(description = "设施类型: ELEVATOR=电梯 ESCALATOR=扶梯 STAIR=楼梯 TOILET=卫生间 ENTRANCE=商场出入口 EXIT=车库出口 SERVICE_DESK=服务台 NURSING_ROOM=母婴室 ATM=取款机")
    ...
    @Schema(description = "设施轮廓（PostGIS几何, GeoJSON; 通用CRUD不读写, 走专门空间接口）")
    ...
    @Schema(description = "设施中心点（PostGIS几何, GeoJSON; 通用CRUD不读写, 走专门空间接口）")
    @TableField(select = false, insertStrategy = FieldStrategy.NEVER, updateStrategy = FieldStrategy.NEVER)
    private String centerPoint;

    /** 设施入口点（PostGIS几何, GeoJSON; 通用CRUD不读写, 走专门空间接口） */
    @Schema(description = "设施入口点（PostGIS几何, GeoJSON; 通用CRUD不读写, 走专门空间接口）")
    @TableField(select = false, insertStrategy = FieldStrategy.NEVER, updateStrategy = FieldStrategy.NEVER)
    private String entrancePoint;

    /** 设施状态: 1=可用 0=不可用 */
    @Schema(description = "设施状态: 1=可用 0=不可用")

 ...
 * 商铺表
 */
@Schema(description = "商铺表")
@Data
@TableName("shop")
public class Shop {

    /** 商铺ID（雪花ID，应用层生成） */
    @Schema(description = "商铺ID（雪花ID，应用层生成）")
    ...
    /** 商铺编号，如 1F-101 */
    @Schema(description = "商铺编号，如 1F-101")
    private String shopNo;

    /** 商铺名称（搜索主字段） */
    @Schema(description = "商铺名称（搜索主字段）")
    ...
    /** 商铺Logo图片地址 */
    @Schema(description = "商铺Logo图片地址")
    private String logoUrl;

    /** 商铺简介 */
    @Schema(description = "商铺简介")
    ...
    @Schema(description = "商铺轮廓（PostGIS几何, GeoJSON; 通用CRUD不读写, 走专门空间接口）")
    ...
    @Schema(description = "商铺中心点（PostGIS几何, GeoJSON; 通用CRUD不读写, 走专门空间接口）")
    @TableField(select = false, insertStrategy = FieldStrategy.NEVER, updateStrategy = FieldStrategy.NEVER)
    private String centerPoint;

    /** 商铺入口点（PostGIS几何, GeoJSON; 通用CRUD不读写, 走专门空间接口） */
    @Schema(description = "商铺入口点（PostGIS几何, GeoJSON; 通用CRUD不读写, 走专门空间接口）")
    @TableField(select = false, insertStrategy = FieldStrategy.NEVER, updateStrategy = FieldStrategy.NEVER)
    private String entrancePoint;

    /** 商铺状态: OPEN=营业 DECORATING=装修 CLOSED=关闭 */
    @Schema(description = "商铺状态: OPEN=营业 DECORATING=装修 CLOSED=关闭")

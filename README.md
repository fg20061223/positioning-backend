...
| 服务 | 注册名 | 端口 | 数据库架构 |
| --- | --- | --- | --- |
| positioning-gateway | positioning-gateway | 8080 | - |
...
默认数据库账号 `postgres/postgres`，可通过环境变量覆盖：`DB_HOST`、`DB_PORT`、`DB_NAME`、`DB_USERNAME`、`DB_PASSWORD`、`NACOS_ADDR`、`SA_TOKEN_SECRET`。
...
# 携带 token 访问业务接口（分页）
curl -X POST http://127.0.0.1:8080/business/mall/page \
  -H "Content-Type: application/json" -H "satoken: <token>" \
  -d '{"pageNum":1,"pageSize":10}'

# 附近空闲车位（楼层本地米制坐标 x,y）
curl -X POST http://127.0.0.1:8080/business/space/nearby \
  -H "Content-Type: application/json" -H "satoken: <token>" \
  -d '{"mallId":1,"x":10.5,"y":20.3,"limit":5}'

# 路径规划
curl -X POST http://127.0.0.1:8080/business/nav/route \
  -H "Content-Type: application/json" -H "satoken: <token>" \
...
- Sa-Token 使用 JWT 无状态模式，认证服务与业务服务共享 `SA_TOKEN_SECRET`；网关不做鉴权，业务接口由各服务统一校验。
